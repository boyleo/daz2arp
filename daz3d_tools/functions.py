...
def snap_arp_ref_bones_to_daz_bones(arp_armature_object: bpy.types.Object, daz_armature_object: bpy.types.Object, report: Optional[_ReportFunctionType] = None):
...
def copy_daz_bones(arp_armature_object: bpy.types.Object, daz_armature_object: bpy.types.Object, report: Optional[_ReportFunctionType] = None):
    arp_armature: bpy.types.Armature = arp_armature_object.data
    arp_bones = arp_armature.edit_bones
    daz_armature: bpy.types.Armature = daz_armature_object.data
    daz_bones = daz_armature.edit_bones
...
def fix_arp_bones(arp_armature_object: bpy.types.Object):
    arp_armature: bpy.types.Armature = arp_armature_object.data
    arp_bones = arp_armature.edit_bones
...
def copy_daz_constraints(arp_armature_object: bpy.types.Object, daz_armature_object: bpy.types.Object, report: Optional[_ReportFunctionType] = None):
...
def fix_daz_corrective_shape_keys(arp_armature_object: bpy.types.Object, daz_armature_object: bpy.types.Object, report: Optional[_ReportFunctionType] = None):
    ...
    for mesh_object in daz_armature_object.children_recursive:
        ...
        for fcurve in mesh.shape_keys.animation_data.drivers:
            ...
            if driver.type == 'SUM':
                same_variables: Dict[str, List[bpy.types.DriverVariable]] = {}

                for variable in driver.variables:
                    target_keys: List[str] = [
                        f"{target.data_path}, {getattr(target.id, 'name', '____NONE____')}"
                        for target in variable.targets
                    ]
                    same_variables.setdefault(';'.join(target_keys), []).append(variable)

                for variables in same_variables.values():
                    if len(variables) == 1:
                        continue
...
def remap_daz_corrective_shape_keys(arp_armature_object: bpy.types.Object, daz_armature_object: bpy.types.Object, report: Optional[_ReportFunctionType] = None):
    mesh_object: bpy.types.Object
    for mesh_object in daz_armature_object.children_recursive:
        if mesh_object.type != 'MESH':
            continue
...
def remap_daz_vertex_groups(daz_mesh_object: bpy.types.Object, report: Optional[_ReportFunctionType] = None):
    vertex_groups = daz_mesh_object.vertex_groups
    for daz_name, arp_name in DAZ_TO_ARP_VERTEXGROUPS.items():
        if daz_name not in vertex_groups:
            if report:
    ...
    def combine_vertex_group(arp_vertex_group_name: str, daz_vertex_group_name: str):
        if arp_vertex_group_name not in vertex_groups or daz_vertex_group_name not in vertex_groups:
            return
